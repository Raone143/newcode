...
function _rescaleGraph(g) {
	...
	for ( var i = 0; i < g.rows.length; i++) {
		for ( var j = 0; j < g.rows[i].length; j++) {
			g.c++;
			if (g.rows[i][j] > g.posMax)
				g.posMax = g.rows[i][j];
			if (g.rows[i][j] < g.negMax)
				g.negMax = g.rows[i][j];
		}
	}
...
function _buildRegGraph(g, doc) {
	...
	if (g.yLabel) {
		...
		str += "<TR>\n";
		var r = 2;
		if (g.negMax && g.posMax)
			r++;
		str += "<TH ROWSPAN=" + r + " ALIGN=LEFT WIDTH=20 NOWRAP>\n";
		str += "<FONT FACE='Arial,Helvetica' SIZE=-1>" + g.yLabel
				+ "</FONT></TD>\n";
	}
	if (g.posMax > 0) {
		if (!g.yLabel)
			str += "<TR>\n";
		if (g.scale)
			str += _writeScaleGraph(g, 0, g.posMax);
		str += "<TD VALIGN=BOTTOM";
		if (g.bgColor)
			str += " BGCOLOR=\"" + g.bgColor + "\"";
		str += ">";
		for ( var j = 0; j < g.rows[0].length; j++) {
			for ( var i = 0; i < g.rows.length; i++) {
				if (parseInt(g.vscale * g.rows[i][j]) > 0) {
					...
					str += "<IMG SRC=\"clear.gif\" WIDTH=" + parseInt(g.hscale)
							+ " HEIGHT=5>";
				str += "</TD>\n<TD VALIGN=BOTTOM";
				if (g.bgColor)
					str += " BGCOLOR=\"" + g.bgColor + "\"";
	...
	if (g.negMax < 0) {
		if (g.posMax != 0 && !g.scale)
			str += "</TR>";
		str += "<TR>\n";
		if (g.scale)
			str += _writeNegScaleGraph(g, g.negMax, 0);
		str += "<TD VALIGN=TOP";
		if (g.bgColor)
			str += " BGCOLOR=\"" + g.bgColor + "\"";
		str += ">";
		for ( var j = 0; j < g.rows[0].length; j++) {
			for ( var i = 0; i < g.rows.length; i++) {
				if (parseInt(g.vscale * g.rows[i][j]) < 0) {
					str += "<IMG VSPACE=0 HSPACE=0 BORDER=0 ALIGN=TOP SRC=" + i
							+ ".gif WIDTH=" + parseInt(g.hscale) + " HEIGHT="
					...
					str += "<IMG SRC=\"clear.gif\" ALIGN=TOP BORDER=0 WIDTH="
							+ parseInt(g.hscale) + " HEIGHT=5>";
				str += "</TD>\n<TD VALIGN=TOP";
				if (g.bgColor)
					str += " BGCOLOR=\"" + g.bgColor + "\"";
				str += ">";
	...
	if (g.xLabel) {
		...
		str += "<TH COLSPAN="
				+ g.c
				+ " HEIGHT=25 VALIGN=BOTTOM><FONT FACE='Arial,Helvetica' SIZE=-1>";
		str += g.xLabel;
		str += "</FONT></TH></TR>\n";
	}
	str += "</TABLE>\n";
	...
	for ( var i = 0; i < g.rows[0].length; i++) {
		...
		if (g.sDate) {
			...
			if (g.longDate && g.showDate) {
				...
				dateStr += (g.sDate.getMonth() + 1) + "/" + g.sDate.getDate();
			...
			if (g.showYear) {
				if (g.longYear)
					dateStr += g.sDate.getFullYear();
				else
					dateStr += (g.sDate.getFullYear() % 100);
			}
...
function _writeScaleGraph(g, min, max) {
	...
	for ( var i = 0; i < n; i++) {
		st += "<FONT FACE=Arial,Helvetica SIZE=-3><I>"
				+ (g.scale * (n - 1) - g.scale * i + g.offset);
		if (g.relative)
			st += "%";
		st += "</I></FONT>";
		st += "<IMG SRC=clear.gif WIDTH=1 HEIGHT=" + H + "><BR>\n";
	...
	if (g.posMax > 0) {
		...
		for ( var j = 0; j < g.rows[0].length; j++) {
			...
			if (g.rows.length > 0 && g.relative && (g.height > k)) {
				str += "<IMG SRC=0.gif WIDTH=" + parseInt(g.hscale)
						+ " HEIGHT=" + (g.height - k) + " ";
				str += "ALT=\"";
				if (g.legends && g.legends[0])
					str += g.legends[0] + ": ";
			...
			for ( var i = y; i < g.rows.length; i++) {
				if (parseInt(g.vscale * g.rows[i][j]) > 0) {
					str += "<IMG SRC=" + i + ".gif WIDTH=" + parseInt(g.hscale)
							+ " HEIGHT=" + parseInt(g.vscale * g.rows[i][j])
							+ " ";
					str += "ALT=\"";
					if (g.legends && g.legends[i])
						str += g.legends[i] + ": ";
