...
class AccessDataBaseClass {
    ...
    function get_information($table_name, $fields = "", $where = "", $order = "", $start = "", $rec_limit = "") {
        ...
        $res_information = pg_query($db_connect, $sel_information);

        while($rows = pg_fetch_assoc($res_information)){
            $table_data[] = $rows;
        }
    ...
    function get_total_records_count($table_name, $where = '') {
        ...
        $res_total_records_count = pg_query($db_connect, $sel_total_records_count);
        $row_information  = pg_fetch_assoc($res_total_records_count);
    ...
    function get_table_information($table_name) {
        ...
        $res_table_information = pg_query($db_connect, $sel_table_information);

        while($rows = pg_fetch_assoc($res_table_information)) {
    ...
    function get_all_tables_in_database() {
        ...
        $res_all_tables = pg_query($db_connect, $get_all_tables);

        while($rows = pg_fetch_row($res_all_tables)) {
            $tables_list[] = $rows[0];
        }
        return $tables_list;
    }
